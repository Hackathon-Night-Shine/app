...
const shouldHideAdminPage = (adminOnlyRoutes: string[], path: string, role: string | undefined) => {
...
const Navbar: React.FC = () => {
    ...
    useEffect(() => {
        if (shouldHideAdminPage(adminOnlyRoutes, location.pathname, currentUser?.role)) {
            navigate('/');
        }
            ...
            >
                {pages.map((page) => (
                    <React.Fragment key={page.path}>
                        {shouldHideAdminPage(adminOnlyRoutes, page.path, currentUser?.role) ? (
                            <></>
                        ) : (
                            <Link
                                key={page.path}
                                ...
                                style={{
                                    textDecoration: 'none',
                                    color: location.pathname === page.path ? '#8B0000' : 'inherit',
                                }}
                            >
                                {page.name}
                            </Link>
