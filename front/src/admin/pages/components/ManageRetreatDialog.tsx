...
const ManageRetreatDialog: React.FC<
    DialogProps<Retreat | LocallyCreatedRetreat, Retreat | LocallyCreatedRetreat | undefined>
> = ({ open, payload, onClose }) => {
    const [tempRetreat, setTempRetreat] = useState<Retreat | LocallyCreatedRetreat>(payload);
    const imageRef = useRef<HTMLInputElement>(null);

    const handleInputfieldChange = (fieldName: string, transform?: (value: string) => any) => {
    ...
    const handleImageUpdate = (imageSrc: string) => {
        setTempRetreat((prevState) => (prevState ? { ...prevState, imageSrc } : prevState));
    };
                ...
                <Grid2 container spacing={2} justifyContent='center'>
                    ...
                    <Grid2 direction='column' gap={2} justifyContent='space-around'>
                        ...
                        <TextField
                            ...
                            onChange={handleInputfieldChange('maximumParticipantsAmount', parseInt)}
                        />
                        <Button
                            variant='contained'
